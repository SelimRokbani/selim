...
contract bet is ATM, Ownable {
    ...
    constructor() payable {
        ...
        teams.push(Team("teamA", 0));
        teams.push(Team("teamB", 0));
    ...
    function teamWinDistribution(uint _teamId) public payable onlyOwner() {
        ...
        if (_teamId == 0) {
            for (uint i = 0; i < bets.length; i++) {
                if (keccak256(abi.encodePacked((bets[i].teamBet.name))) == keccak256(abi.encodePacked("teamA"))) {
                    address payable receiver = payable(bets[i].adres);
            ...
            for (uint i = 0; i < bets.length; i++) {
                if (keccak256(abi.encodePacked((bets[i].teamBet.name))) == keccak256(abi.encodePacked("teamB"))) {
                    address payable receiver = payable(bets[i].adres);
